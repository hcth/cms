...
class Package extends MX_Controller
{
	...
	*/
	...
	public function listpackage()
	{
		...
		if (!empty($isAdmin)) :
			...
			foreach ($disposition_detail as $d_v) {
				$record['disposition_detail'][$d_v->disposition] = $d_v->disposition_count;
			}
	...
	public function download_packagecsv()
	{
		$query = "
			SELECT 
				package_detail.id as id ,
				user.name as AssignedToName,
				package_detail.traveller_code as TravellerCode,
				package_detail.landing as Destination,
				package_detail.platform as Platform,
				package_detail.campaigntype as CampaignType,
				package_detail.adgroup as AdvertiseGroup,
				package_detail.created_date as LeadDate,
				package_detail.name as Name,
				package_detail.email as Email,
				package_detail.phone as Mobile ,
				package_detail.package as Package,
				package_detail.disposition as Disposition,
				package_detail.sales_status as SalesStatus
			FROM
			package_detail LEFT JOIN user on user.id=package_detail.assigned_to
		";
		$record['resultdata'] = $this->admin_model->get_query($query);
	...
	public function maldives($p1)
	{
		if ($p1 != '') {
			if ($p1 == "gsmle01") {
				$record['platform'] 	= "Google";
				$record['ct'] 			= "Search";
				$record['destination'] 	= "Maldives";
				$record['ad'] 			= "Maldives-GS-MLE-01";
			...
			$record['title'] = "Adventourist: Road Trips | Things To Do | Stories.";
			$record['descriptions'] = "Team adventourist, adventourist team members chirag joshi and other members behind the establishment of adventourist.";
			$record['keywords'] = "adventourist team, team adventourist, adventourist employees, adventourist travellers, chirag joshi.";
			$record['viewFile'] = "package";
			$record['module'] = "package";
	...
	{
		$template = "thailand";
		$record['destination'] = "Thailand";
		$record['platform'] = "Google";
		$record['ct'] = "Search";
		$record['ad'] = "thailand-gs-th01";
	...
	{
		$template = "northeast";
		$record['destination'] = "North East";
		$record['platform'] = "Google";
		$record['ct'] = "Search";
		$record['ad'] = "northeast-gs-ne01";
